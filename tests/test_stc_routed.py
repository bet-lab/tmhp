...
def test_construction_type_check():
    with pytest.raises(TypeError):
        GSHPB_STC_routed(stc=object(), **_CFG)
    m = GSHPB_STC_routed(stc=_stc(), **_CFG)
...
def test_zero_sun_matches_base():
    ...
    rt = _run(GSHPB_STC_routed(stc=_stc(), **_CFG), sun=False)
...
def test_force_ground_route_charges_ground_only():
    ...
    model = GSHPB_STC_routed(stc=_stc(), solar_router=lambda **_: "ground", **_CFG)
    ...
    base = _run(GSHPB_STC_routed(stc=_stc(), solar_router=lambda **_: "ground", **_CFG), sun=False)
...
def test_force_tank_route_charges_tank_only():
    ...
    model = GSHPB_STC_routed(stc=_stc(), solar_router=lambda **_: "tank", **_CFG)
    df = _run(model, sun=True)
...
def test_routing_is_exclusive_never_both():
    ...
    df = _run(GSHPB_STC_routed(stc=_stc(), T_tank_w_lower_bound=60.0, **_CFG), sun=True)
...
def test_invalid_route_raises():
    model = GSHPB_STC_routed(stc=_stc(), solar_router=lambda **_: "bogus", **_CFG)
