...
Solar thermal collectors, photovoltaic systems, and energy storage
are not modelled as separate top-level classes — they plug into an
existing heat-pump model on the demand side, augmenting (not
replacing) the refrigerant cycle. This tutorial walks through the
simplest such composition: ``ASHPB_STC_preheat``, where a
``SolarThermalCollector`` heats the cold mains water before it
reaches the DHW tank.

The two pieces
==============

The :class:`~tmhp.subsystems.SolarThermalCollector` is a
...
:class:`~tmhp.ashpb_stc_preheat.ASHPB_STC_preheat` extends
...
A look at the PV + ESS ledger
=============================

The same composition pattern applies to ``ASHPB_PV_ESS``: instantiate
a :class:`~tmhp.subsystems.PhotovoltaicSystem` and an
:class:`~tmhp.subsystems.EnergyStorageSystem`, hand them to the model,
and drive ``analyze_dynamic`` with an irradiance schedule on top of
the usual ones. The figure below is what a day of that looks like.
...
.. figure:: ../_static/pv_ess_energy_balance.svg
    ...
    and the default ``PhotovoltaicSystem`` / ``EnergyStorageSystem``
    sizes. Panel (b) makes the sizing tradeoff readable: shrinking
    the ESS column on the left would push more of the right-hand
    bar from "PV" to "Grid". Generated by
...
- :class:`~tmhp.ashpb_stc_tank.ASHPB_STC_tank` — STC charges
  the top node of a stratified storage tank instead of the
  mains feed.
- :class:`~tmhp.ashpb_pv_ess.ASHPB_PV_ESS` — photovoltaic
  generation + ESS supplies compressor and auxiliary loads
  before drawing grid power.
- The :class:`~tmhp.gshpb_stc_preheat.GSHPB_STC_preheat`,
  :class:`~tmhp.gshpb_stc_tank.GSHPB_STC_tank`, and
  :class:`~tmhp.gshpb_pv_ess.GSHPB_PV_ESS` mirrors on the
  ground-source side.
