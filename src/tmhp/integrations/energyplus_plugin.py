...
:class:`TmhpPlantSurrogate`, a ``PythonPlugin:Variables`` global ``tmhp_E_cmp``,
and a ``PythonPlugin:OutputVariable`` routing it to a meter.
...
class TmhpPlantSurrogate(EnergyPlusPlugin):
    ...
    ``tmhp_E_cmp`` (declare it in ``PythonPlugin:Variables`` and meter it via
    ...
    def _get_handles(self, state: Any) -> None:
        ...
        self.h = dict(
            ...
            e_cmp=ex.get_global_handle(state, "tmhp_E_cmp"),
        )
        self._need = False
    ...
    def on_user_defined_component_model(self, state: Any) -> int:
        ...
        if q_req < 1.0:
            ...
            ex.set_global_value(state, self.h["e_cmp"], 0.0)
            return 0
